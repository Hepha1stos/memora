...
export async function POST({request}: { request: Request }){
  ...
  try{
    await db.insert(learnSession).values({category_id:response.category_id,total_correct:response.total_correct, total_wrong:response.total_wrong, data:response.data})
  } catch (error){
